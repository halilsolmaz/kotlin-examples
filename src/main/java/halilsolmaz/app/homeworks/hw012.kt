/*-----------------------------------------------------------------------------------------------------
    ...
    numToStr3DigitsTR012 fonksiyonunu yazınız ve aşağıdaki kod ile test ediniz.
    Açıklamalar:
...
fun runNumToStrTest()
{
    ...
    for (i in 1..count) {
        ...
        println("$value -> ${numToStr3DigitsTR012(value)}")
    }

    println("Tekrar yapıyor musunuz?")
}

val g_onesTR012 = arrayOf("", "bir", "iki", "üç", "dört", "beş", "altı", "yedi", "sekiz", "dokuz")
val g_tensTR012 = arrayOf("", "on", "yirmi", "otuz", "kırk", "elli", "altmış", "yetmiş", "seksen", "doksan")

fun numToStr3DigitsTR012(value: Int) : String
{
    ...
    {
        if(arr[0] >= 2)
            str.append(g_onesTR012[arr[0]])
        if(arr[0] >= 1)
            str.append("yüz")
        str.append(g_tensTR012[arr[1]] + g_onesTR012[arr[2]])
    } else if (arr.size == 2){
        str.append(g_tensTR012[arr[0]] + g_onesTR012[arr[1]])
    } else {
        if (value == 0) str.append("sıfır") else str.append(g_onesTR012[abs(value)])
    }
